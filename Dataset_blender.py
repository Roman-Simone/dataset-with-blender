...
ROOT = "/Users/simoneroman/Desktop/VISION/BLENDER/DATA" # Dataset location
...
class Render:
    def __init__(self):
        ...
        for item in bpy.data.objects:
            if item.name in ["Scene","Camera","Main Axis","Light1","Light2","tavolo"]:
                continue       
            self.obj.append(item)
            nome_mat = f"{item.name}_mat"
            if nome_mat in item.data.materials:
                mat = bpy.data.materials.new(name=f"{item.name}_mat") #set new material to variable
                item.data.materials.append(mat) #add the material to the object


        self.images_filepath = ROOT
    ...
    def set_camera(self):
        self.camera.rotation_euler = (math.radians(65), math.radians(2), math.radians(-90))
    ...
    def get_all_coordinates(self):
        ...
        for i in range(0, 10+1): # Loop through all of the objects
            ...
            if self.obj[i].hide_viewport!=False:
                continue
            b_box = self.find_bounding_box(self.obj[i]) # Get current object's coordinates
            if b_box: # If find_bounding_box() doesn't return None
                #print("         Initial coordinates:", b_box)
                text_coordinates = self.format_coordinates(b_box, self.obj[i].name) # Reformat coordinates to YOLOv3 format
                #print("         YOLO-friendly coordinates:", text_coordinates)
                main_text_coordinates = main_text_coordinates + text_coordinates # Update main_text_coordinates variables whith each                                                                             # line corresponding to each class in the frame of the current image
            else:
        ...
        for i in range(n_objects+1):
            ...
            while(not pos_find):               
                ...
                for item in self.obj:
                    ...
                    dist = pow(pow(x-item.location[0],2)+pow(y-item.location[1],2),0.5)
                    if dist < dim1 + dim2:
                        pos_find = False
                        break
            ...
            self.obj[i].active_material.diffuse_color = self.choose_color() #change color
            ...
            self.obj[i].rotation_euler = (math.radians(angle_x),math.radians(angle_y),math.radians(angle_z))
    ...
    def render_blender(self, count_image):
        ...
        self.export_render(self.xpix, self.ypix, self.percentage, self.images_filepath, image_name)
